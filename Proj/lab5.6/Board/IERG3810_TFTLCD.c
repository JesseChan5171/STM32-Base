...
void IERG3810_ShowSID(char* str, u16 y, u16 color, u16 bgcolor, u8 x_off)
{
	u8 x = 0;
	u8 i = 0;
	u8 x_inc = 8;
	while(str[i] !='\0'){
		//u8 send_ascii = str[i];
		IERG3810_TFTLCD_ShowChar(x_off+x_inc*i, y, (int) str[i], color, bgcolor);
		i++;
	}
}

void IERG3810_ShowName(u16 y, u16 color, u16 bgcolor)
{
	u8 x = 0;
	u8 i = 0;
	u8 x_inc = 16;
	...
	for(i = 0; i < 3; i++){
	IERG3810_TFTLCD_ShowChinChar(70+x_inc*i, y,i,color,bgcolor);
	}
}
